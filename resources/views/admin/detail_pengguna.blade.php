...
@section('link-breadcrumb', 'Detail')
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li>
                        <div class="flex items-center">
                            <a href="/pengguna-admin"
                                class="ml-1 text-sm font-medium  hover:text-main md:ml-2 text-gray-400">Pengguna</a>
                        </div>
                    </li>
...
@endsection
    ...
    <main class="">
        <article class="">
            {{-- <section class="flex flex-row items-center bg-[#FAFAFA] my-6 p-4 py-6 rounded-md "> 
                <div class="flex flex-row items-center ">    
                    <div class="flex flex-row items-center">
                        <span class="mx-6 text-base text-silver-50">Pengguna</span>
                        <i class="fa-solid fa-chevron-right text-xs text-silver-50"></i>
                        <span class="mx-6 text-base text-silver-50">Detail</span>

                    </div>
                    
                    <a href="/notifikasi-pembayaran" class="absolute right-40 text-lg">
            ...
            <section>
                <div class="bg-[#00BF71] w-full rounded-md relative">
                    <img src="./assets/image/avatar/avatar2.png" alt="" class="w-36 absolute left-12 top-8 rounded-full ring-4 ring-[#FAFAFA]">
                    ...
                    <div class="flex flex-row relative px-12 py-6">
                        <div class="flex flex-col justify-center ml-40">
                            <span class="text-3xl text-[#ECECEC] font-bold my-2 mt-4">
                                Biru Muluk Hadiawan
                            </span>
                    ...
                    <div class="bg-[#FAFAFA] px-12 pt-20">
                        <div class="flex flex-row ">
                            <div class="flex flex-col gap-8 w-96 mr-56">
                                ...
                                <form class="flex flex-col gap-10">
                                    ...
                                    <div class="relative z-0 w-full group">
                                        <input type="text" name="tanggal" id="tanggal" class=" block p-4 w-full text-base text-silver-50 bg-transparent border-2 border-[#AFAFAF] rounded-md border-opacity-50 dark:focus:border-[#00BF71] focus:outline-none focus:ring-0 focus:border-[#00BF71] peer pl-7" placeholder=" " required />
                                        <label for="tanggal" class="peer-focus:font-medium absolute text-sm text-[#AFAFAF] bg-[#FAFAFA] px-2 duration-300 transform -translate-y-7 scale-90 top-5 left-5 z-10 peer-focus:text-[#00BF71] peer-focus:dark:text-[#00BF71] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-7 peer-focus:bg-[#FAFAFA] ">Tanggal Lahir</label>
                                        <div id="error-message-tanggal">
                                            <img src="./assets/image/svg/alert.svg" alt="" class="absolute right-5 top-5">
                                            <span class="text-[#FF6A69] text-xs">Error : Tanggal Tidak Boleh Kosong</span>
                                        </div>
                                    </div>
                                    <div class="relative z-0 w-full group">
                                        <input type="number" name="nomer" id="nomer" class="block p-4 w-full text-base text-silver-50 bg-transparent border-2 border-[#AFAFAF] rounded-md border-opacity-50 dark:focus:border-[#00BF71] focus:outline-none focus:ring-0 focus:border-[#00BF71] peer pl-7" placeholder=" " required />
                                        <label for="nomer" class="peer-focus:font-medium absolute text-sm text-[#AFAFAF] bg-[#FAFAFA] px-2 duration-300 transform -translate-y-7 scale-90 top-5 left-4 z-10 peer-focus:text-[#00BF71] peer-focus:dark:text-[#00BF71] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-7 peer-focus:bg-[#FAFAFA] ">No. Telephone
                                        </label>
                                        <div id="error-message-nomer">
                                            <img src="./assets/image/svg/alert.svg" alt="" class="absolute right-5 top-5">
                                            <span class="text-[#FF6A69] text-xs">Error : No Telephone Tidak Boleh Kosong</span>
                                        </div>
                                    </div>
                                </form>
                            </div>
                        ...
                        <div class="flex justify-center">
                            <button type="button" id="submit-button" class="bg-[#00BF71] py-3 px-6 my-6 mt-12 rounded-md text-[#FFFFFF] font-semibold  hover:bg-[#008753] hover:text-[#FFFFFF] ">Perbarui</button>
                        </div>
                        <script>
                        ...
                        <script>
                            ...
                            // Mengambil elemen span error
                            // const namaErrorMessage = document.getElementById('error-message-nama');
                            const tanggalErrorMessage = document.getElementById('error-message-tanggal');
                            const nomerErrorMessage = document.getElementById('error-message-nomer');
                            ...
                            //     namaErrorMessage.style.display = namaInput.value ? 'none' : 'block';
                            // });

                            // Mendeteksi perubahan pada input
                            tanggalInput.addEventListener('input', function () {
                                // Menyembunyikan pesan error jika input diisi
                                tanggalErrorMessage.style.display = tanggalInput.value ? 'none' : 'block';
                            });

                            // Mendeteksi perubahan pada input
                            nomerInput.addEventListener('input', function () {
                                // Menyembunyikan pesan error jika input diisi
                               nomerErrorMessage.style.display = nomerInput.value ? 'none' : 'block';
                            });
                        ...
                        <script>
                            ...
                            submitButton.addEventListener('click', function () {
                                ...
                                const emailValue = document.getElementById('nama').value;
                                const emailValue = document.getElementById('tanggal').value;
                                const emailValue = document.getElementById('nomer').value;
                                ...
                                // Misalnya, kirim data ke server menggunakan AJAX atau simpan ke database

                                // Contoh: Menampilkan data di konsol
                                console.log('nama:', emailValue);
                                console.log('tanggal:', emailValue);
                                console.log('nomer:', emailValue);
                                console.log('Email:', emailValue);
                                console.log('Kota:', kotaValue);
                                console.log('Alamat:', alamatValue);

                                // Jika Anda ingin mengirim data ke server, gunakan metode fetch atau AJAX di sini
                                // fetch('URL_SERVER', {
                                //     method: 'POST',
                                //     body: JSON.stringify({ email: emailValue, kota: kotaValue, alamat: alamatValue }),
                                //     headers: {
